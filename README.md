...
按你的反馈，**去掉第二步清洗**：第一步 SQL 写出的 Hive 边表直接给随机游走使用。
...
1. `scripts/01_prepare_sequences.py`
   ...
   - 按 `--dt` 替换 SQL 中的日期占位符
   - SQL 内部负责写入 Hive（你当前版本的 SQL 已写 Hive，字段通常为 `id1`,`id2`,`weight`）
2. `scripts/03_generate_walks.py`
   - 直接读取第一步产出的 Hive 边表
   - 生成加权随机游走序列并写 Hive
3. `scripts/04_train_embeddings.py`
   - 读取游走 Hive 表，训练 `Word2Vec`，写 embedding Hive 表
4. `scripts/05_build_recall.py`
...
> 说明：1/3/4/5 步都依赖 Hive 元数据权限，请确保 SparkSession 已启用 Hive 支持。
